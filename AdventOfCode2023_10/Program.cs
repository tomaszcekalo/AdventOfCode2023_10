...
var input = @"L.F--FF|.-.FF-777.F7FL-F.7.F-77.F--FF--.JJ7..F-77|FFF|7-F-J7--F7|7-F7FJ7-F7FF--7--7F-|7F--.|FF|7FF-F77-F7FJ777.FJ.FJ7-|-7-FFFJFF-77L7LF|FL77
...
{
    public void FindLoop(string input)
    {
        input = input
            .Replace('L', '└')
            .Replace('J', '┘')
        ...
        List<PipeItem> pipeItems = new List<PipeItem>();
        pipeItems.Add(new PipeItem()
        {
            Coordinates = startingPoint,
            DistanceFromStart = 0,
            IsStart = true,
            PipeChar = lines[startingPoint.Y][startingPoint.X]
        });
        if (ConnectsSouth(lines[startingPoint.Y - 1][startingPoint.X]))
        {
            pipeItems.Add(new PipeItem()
            {
                Coordinates = new Point(startingPoint.X, startingPoint.Y - 1),
                DistanceFromStart = 1,
                PipeChar = lines[startingPoint.Y - 1][startingPoint.X]
            });
        }
        if (ConnectsNorth(lines[startingPoint.Y + 1][startingPoint.X]))
        {
            pipeItems.Add(new PipeItem()
            {
                Coordinates = new Point(startingPoint.X, startingPoint.Y + 1),
        ...
        if (ConnectsWest(lines[startingPoint.Y][startingPoint.X + 1]))
        {
            pipeItems.Add(new PipeItem()
            {
                Coordinates = new Point(startingPoint.X + 1, startingPoint.Y),
                DistanceFromStart = 1,
                PipeChar = lines[startingPoint.Y][startingPoint.X + 1]
            });
        }
        if (startingPoint.X > 0 && ConnectsEast(lines[startingPoint.Y][startingPoint.X - 1]))
        {
            pipeItems.Add(new PipeItem()
            {
                Coordinates = new Point(startingPoint.X - 1, startingPoint.Y),
        ...
        PipeItem currentPipeItem = null;
        while (true)
        {
            var last = pipeItems.Last();
            if (currentPipeItem == null)
                currentPipeItem = last;
            else
                currentPipeItem = pipeItems
                    .Where(x => x != currentPipeItem)
                    .Where(x => x != last)
                    .OrderBy(x => x.DistanceFromStart)
                    .Last();
            var currentPoint = currentPipeItem.Coordinates;
            var currentChar = lines[currentPoint.Y][currentPoint.X];
            var pointsToCheck = GetPointsToCheck(currentPoint, currentChar);
            var nextCoordinates = pointsToCheck.FirstOrDefault(p => !pipeItems.Any(x => x.Coordinates == p));
            if (nextCoordinates == Point.Empty)
            {
                break;
            }
            var nextPipeItem = new PipeItem()
            {
                Coordinates = nextCoordinates,
                DistanceFromStart = currentPipeItem.DistanceFromStart + 1,
                PipeChar = lines[nextCoordinates.Y][nextCoordinates.X]
            };
            pipeItems.Add(nextPipeItem);
        }
        var newLines = lines.Select(x => new string(' ', x.Length)).ToArray();
    ...
    public List<Point> GetPointsToCheck(Point currentPoint, char currentChar)
    {
        ...
        if (ConnectsSouth(currentChar))
        {
            points.Add(new Point(currentPoint.X, currentPoint.Y + 1));
        }
        if (ConnectsNorth(currentChar))
        {
            points.Add(new Point(currentPoint.X, currentPoint.Y - 1));
        }
        if (ConnectsEast(currentChar))
        {
            points.Add(new Point(currentPoint.X + 1, currentPoint.Y));
        }
        if (ConnectsWest(currentChar))
        {
            points.Add(new Point(currentPoint.X - 1, currentPoint.Y));
        }
        return points;
    ...
    public bool ConnectsSouth(char c)
    {
        return c == '┌' || c == '┐' || c == '│';
    }

    public bool ConnectsNorth(char c)
    {
        return c == '└' || c == '┘' || c == '│';
    }

    public bool ConnectsEast(char c)
    {
        return c == '└' || c == '┌' || c == '─';
    }

    public bool ConnectsWest(char c)
    {
        return c == '┘' || c == '┐' || c == '─';
    }
}